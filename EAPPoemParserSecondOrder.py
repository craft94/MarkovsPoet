...
Shakespeare = open("PoePoemsOneLine.txt", "r")
...
def Main():
	...
	for l in range(0,6):
		for w in range(0,2):
			print(List[CurrentWord] + " " + List[CurrentWord + 1] + " " + List[CurrentWord + 2] + " ", end="")
			PlaceOfWord = np.where(np.asarray(List)==List[CurrentWord + 2])
